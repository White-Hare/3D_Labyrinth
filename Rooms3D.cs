...
public class Rooms3D : Labyrinth3D
{
    ...
    protected override void GenerateMaze()
    {
        ...
        Color[] colors = new Color[]{Color.black, Color.cyan, Color.blue, Color.red, Color.magenta, Color.yellow, Color.grey};
        ...
        //Create Room
        void Carve(List<Cell> room, Cell cell, int xOffset = 0, int yOffset = 0, int zOffset = 0)
        {
        ...
        foreach (Cell c in room)
        {
            ...
            if (c.x < columns - 1) Carve(room, cells[c.x + 1, c.y, c.z], 0, 1, 1);
            if (c.x > 0) Carve(room, cells[c.x - 1, c.y, c.z], 2, 1, 1);

            if (c.y < layers - 1) Carve(room, cells[c.x, c.y + 1, c.z], 1, 0, 1);
            if (c.y > 0) Carve(room, cells[c.x, c.y - 1, c.z], 1, 2, 1);

            if (c.z < rows - 1) Carve(room, cells[c.x, c.y, c.z + 1], 1, 1, 0);
            if (c.z > 0) Carve(room, cells[c.x, c.y, c.z - 1], 1, 1, 2);
            ...
            for (int z = 0; z < 2; z++)
            {
                bool xm = x > 0 ? c.x < columns - 1 : c.x > 0;
                bool ym = y > 0 ? c.y < layers - 1 : c.y > 0;
                bool zm = z > 0 ? c.z < rows - 1 : c.z > 0;

                if (xm && ym && zm && !IsCorner(room, new Cell(c.x * 2, c.y * 2, c.z * 2)))
                        Carve(room, cells[c.x + x, c.y + y, c.z + z]);
            }


            CarveSides(room, c, 0, 0, 1);
            CarveSides(room, c, 1, 0, 0);
            CarveSides(room, c, 2, 0, 1);
            CarveSides(room, c, 1, 0, 2);       
            
            CarveSides(room, c, 0, 2, 1);
            CarveSides(room, c, 1, 2, 0);
            CarveSides(room, c, 2, 2, 1);
            CarveSides(room, c, 1, 2, 2);


            CarveSides(room, c, 0, 1, 0);
            CarveSides(room, c, 2, 1, 0);
            CarveSides(room, c, 0, 1, 2);
            CarveSides(room, c, 2, 1, 2);
